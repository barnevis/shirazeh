...
### استفاده به صورت محلی (Local)

اگر می‌خواهید پروژه شما به صورت کاملاً آفلاین کار کند یا از وابستگی به CDN جلوگیری کنید، می‌توانید کتابخانه Highlight.js را به صورت محلی میزبانی کنید.

**گام اول: دانلود کتابخانه**
1.  به صفحه [دانلود Highlight.js](https://highlightjs.org/download/) بروید.
2.  زبان‌های برنامه‌نویسی مورد نیاز خود را انتخاب کنید (این کار به کاهش حجم نهایی کمک می‌کند).
3.  روی دکمه `Download` کلیک کنید تا فایل فشرده کتابخانه را دریافت کنید.

**گام دوم: قرار دادن فایل‌ها**
1.  فایل دانلود شده را از حالت فشرده خارج کنید.
2.  محتویات آن را در مسیری داخل پروژه خود قرار دهید. مسیر پیشنهادی که با تنظیمات پیش‌فرض هماهنگ است، `src/lib/vendor/highlight` می‌باشد.
3.  ساختار نهایی پوشه باید به شکل زیر باشد:
    ```
    shirazeh/
    └── src/
        └── lib/
            └── vendor/
                └── highlight/       <-- مسیر 'path' در کانفیگ
                    ├── highlight.min.js
                    ├── languages/
                    │   ├── javascript.min.js
                    │   └── ...
                    └── styles/
                        ...
                        └── github-dark.min.css
    ```
    > **نکته مهم:** مطمئن شوید که فایل‌های تم (مانند `github.min.css`) در داخل پوشه `styles` قرار دارند.

**گام سوم: به‌روزرسانی `config.js`**
تنظیمات `highlight` را به شکل زیر تغییر دهید تا از منبع محلی استفاده کند:
```javascript
highlight: {
  enabled: true,
  source: 'local', // <-- تغییر به حالت محلی
  local: {
    path: 'src/lib/vendor/highlight', // مسیر پوشه اصلی کتابخانه
    themeLight: 'github.min.css',
    themeDark: 'github-dark.min.css',
  },
  languages: ['javascript', 'xml', 'css'], // زبان‌هایی که دانلود کرده‌اید
  // ... سایر تنظیمات
},
```
...
-   `showLanguage`: (`boolean`, پیش‌فرض: `true`) نمایش برچسب زبان/نام فایل در بالای بلوک کد را فعال یا غیرفعال می‌کند.
-   `showFileName`: (`boolean`, پیش‌فرض: `true`) نمایش نام فایل (در صورت وجود) در برچسب بالای بلوک کد را فعال یا غیرفعال می‌کند.
...
-   **`[:filename]` (اختیاری):** نام فایلی که می‌خواهید نمایش داده شود (با یک `:` از زبان جدا می‌شود).
