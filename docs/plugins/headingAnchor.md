...
این افزونه یک قابلیت استاندارد و بسیار کاربردی را به وب‌سایت شما اضافه می‌کند: امکان کپی کردن لینک مستقیم به هر بخش از محتوای شما. این کار با افزودن یک آیکون قابل کلیک در کنار عنوان‌ها صفحه (مانند `<h2>`, `<h3>` و ...) انجام می‌شود.
...
-   `levels`:
    ...
    -   **توضیحات:** آرایه‌ای از سطوح عنوان‌های که می‌خواهید لینک به آن‌ها اضافه شود. به صورت پیش‌فرض، به `<h1>` لینک اضافه نمی‌شود، زیرا معمولاً به عنوان عنوان اصلی صفحه در نظر گرفته می‌شود.
