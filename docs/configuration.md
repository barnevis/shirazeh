...
### `logo`
این بخش به شما اجازه می‌دهد تا لوگو و نام ویکی خود را در هدر سایدبار مدیریت کنید.

-   `enabled`: فعال یا غیرفعال کردن نمایش **تصویر لوگو** (مقدار پیش‌فرض: `true`).
-   `showAppName`: فعال یا غیرفعال کردن نمایش **نام متنی ویکی** (مقدار پیش‌فرض: `true`).
-   `src`: مسیر فایل لوگوی شما. برای نمایش لوگو، این فیلد باید مقدار داشته باشد.
...
-   **مثال (فقط نمایش نام):**
    ```javascript
    logo: {
      enabled: false, // لوگو را مخفی کن
      showAppName: true, // نام را نمایش بده
    }
    ```
-   **مثال (فقط نمایش لوگو):**
    ```javascript
    logo: {
      enabled: true,
      showAppName: false, // نام را مخفی کن
...
### `githubCorner`
این بخش افزونه نمایش لینک گیت‌هاب در گوشه صفحه را کنترل می‌کند.
> **نکته مهم:** از نسخه 0.3.0 به بعد، موقعیت این ویجت از طریق **[سیستم مدیریت ویجت‌ها](#widgets)** کنترل می‌شود.

-   `enabled`: فعال یا غیرفعال کردن افزونه (مقدار پیش‌فرض: `false`).
-   `url`: آدرس کامل مخزن گیت‌هاب شما. این فیلد برای فعال شدن افزونه **الزامی** است.
-   `backgroundColor`: رنگ پس‌زمینه آیکون. اگر خالی باشد، از رنگ پیش‌فرض تم (روشن/تاریک) استفاده می‌شود.
-   `iconColor`: رنگ آیکون اختاپوس (octocat). اگر خالی باشد، از رنگ پیش‌فرض تم استفاده می‌شود.
...
این بخش جدید، سیستم قدرتمند مدیریت ویجت‌ها را کنترل می‌کند. این سیستم به شما اجازه می‌دهد تا موقعیت کامپوننت‌های کوچک UI (مانند دکمه تغییر تم یا لینک گیت‌هاب) را به صورت متمرکز و دقیق مدیریت کنید.

-   `enabled`: فعال یا غیرفعال کردن کامل سیستم ویجت. اگر `false` باشد، افزونه‌ها به رفتار پیش‌فرض خود برای موقعیت‌یابی بازمی‌گردند.
-   `slots`: یک آبجکت که موقعیت هر ویجت را مشخص می‌کند. کلید این آبجکت، `id` منحصر به فرد ویجت است.
...
> برای اطلاعات کامل در مورد اسلات‌های موجود، سیاست‌های موبایل و نحوه کارکرد این سیستم، به **[مستندات سیستم مدیریت ویجت‌ها](/docs/widgetSystem)** مراجعه کنید.
