# راهنمای پیکربندی

تمام تنظیمات PWiki از طریق یک فایل جاوااسکریپت به نام `config/config.js` مدیریت می‌شود. این فایل به شما اجازه می‌دهد تا جنبه‌های مختلف ویکی، از مسیر فایل‌ها گرفته تا رنگ‌بندی را به سادگی شخصی‌سازی کنید.

فایل `config.js` به صورت پیش‌فرض شامل تمام گزینه‌های ممکن است. شما فقط باید مقادیر گزینه‌هایی را که می‌خواهید تغییر دهید، ویرایش کنید. اگر گزینه‌ای را حذف کنید، مقدار پیش‌فرض آن استفاده خواهد شد.

## بررسی گزینه‌ها

در ادامه، هر بخش از فایل تنظیمات به تفصیل توضیح داده می‌شود.

### `basePath`

این گزینه مسیر پایه ویکی شما را مشخص می‌کند. اگر پروژه شما مستقیماً در ریشه یک دامنه قرار دارد، مقدار پیش‌فرض `.` مناسب است. اما اگر آن را در یک زیرپوشه (مثلاً `example.com/docs/`) قرار داده‌اید، باید این مقدار را تنظیم کنید.

-   **مقدار پیش‌فرض:** `'.'`
-   **مثال:**
    ```javascript
    basePath: 'my-wiki-docs'
    ```

### `files`

این بخش مسیر فایل‌های اصلی پروژه را تعیین می‌کند. تمام مسیرها نسبت به `basePath` در نظر گرفته می‌شوند.

-   `sidebar`: فایل مارک‌داونی که ساختار منوی کناری را تعریف می‌کند.
-   `defaultPage`: فایلی که به عنوان صفحه اصلی (وقتی کاربر آدرس ریشه را باز می‌کند) نمایش داده می‌شود.

-   **مقدار پیش‌فرض:**
    ```javascript
    files: {
      sidebar: 'config/sidebar.md',
      defaultPage: 'README.md'
    }
    ```
-   **مثال:**
    ```javascript
    files: {
      sidebar: 'navigation/menu.md',
      defaultPage: 'introduction.md'
    }
    ```

### `selectors`

این گزینه‌ها شناسه‌های CSS (سلکتورها) برای المان‌های اصلی HTML هستند. **تنها در صورتی این مقادیر را تغییر دهید که ساختار فایل `index.html` را ویرایش کرده باشید.**

-   **مقدار پیش‌فرض:**
    ```javascript
    selectors: {
      content: '.content',
      sidebarNav: '.sidebar-nav',
      sidebarToggle: '.sidebar-toggle',
    }
    ```

### `sidebar`

تنظیمات مربوط به رفتار منوی کناری.

-   `enabled`: مشخص می‌کند که آیا منوی کناری نمایش داده شود یا خیر. اگر آن را `false` کنید، منو و دکمه آن به طور کامل از صفحه پنهان می‌شوند و محتوای اصلی تمام عرض صفحه را اشغال می‌کند.

-   **مقدار پیش‌فرض:**
    ```javascript
    sidebar: {
      enabled: true,
    }
    ```
-   **مثال (برای غیرفعال کردن منو):**
    ```javascript
    sidebar: {
      enabled: false,
    }
    ```

### `theme`

قدرتمندترین بخش برای شخصی‌سازی ظاهری! در این بخش می‌توانید رنگ‌های پیش‌فرض پروژه را بازنویسی کنید. نام هر متغیر به صورت `camelCase` است که معادل متغیر CSS آن (به صورت `kebab-case`) می‌باشد.

برای مشاهده لیست کامل متغیرهای قابل تغییر، به فایل `src/styles/variables.css` مراجعه کنید.

-   **مقدار پیش‌فرض:** `{}` (بدون هیچ بازنویسی)
-   **مثال (برای تغییر رنگ اصلی به بنفش و پس‌زمینه سایدبار به رنگ تیره):**
    ```javascript
    theme: {
      primaryColor: '#a78bfa',   // رنگ اصلی (بنفش)
      sidebarBg: '#1e1b4b',      // پس‌زمینه سایدبار (سرمه‌ای تیره)
      textColor: '#e5e7eb',      // رنگ متن
    }
    ```

> **نکته مهم:** پس از هر تغییر در فایل `config.js`، کافیست صفحه مرورگر را رفرش کنید تا تغییرات اعمال شوند. نیازی به اجرای مجدد سرور نیست.
