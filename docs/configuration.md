...
### `logo`
این بخش به شما اجازه می‌دهد تا برای ویکی خود یک لوگو تعریف کنید.

-   `enabled`: فعال یا غیرفعال کردن نمایش لوگو (مقدار پیش‌فرض: `false`).
-   `src`: مسیر فایل لوگوی شما. این تصویر هم در هدر و هم به عنوان آیکون تب (favicon) استفاده می‌شود.
