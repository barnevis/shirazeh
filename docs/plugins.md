...
این افزونه به کاربران اجازه می‌دهد تا صفحه فعلی را به فرمت‌های مختلفی مانند PDF، HTML یا Markdown خام دانلود کنند.
برای اطلاعات کامل به [مستندات افزونه خروجی صفحه](/docs/plugins/pageExporter) مراجعه کنید.

### افزونه نمودار Mermaid
این افزونه به شما اجازه می‌دهد تا نمودارها، فلوچارت‌ها و دیاگرام‌های پیچیده را با سینتکس متنی ساده در صفحات خود ترسیم کنید.
برای اطلاعات کامل به [مستندات افزونه Mermaid](/docs/plugins/mermaid) مراجعه کنید.
...
### یکپارچه‌سازی با سیستم مدیریت ویجت‌ها

اگر افزونه شما یک عنصر قابل مشاهده به UI اضافه می‌کند (مانند یک دکمه یا آیکون)، بهترین روش برای موقعیت‌دهی آن، استفاده از **سیستم مدیریت ویجت‌ها** است. این کار به جای افزودن مستقیم عنصر به `document.body` و مدیریت موقعیت آن با CSS اختصاصی انجام می‌شود.

-   **چرا از این سیستم استفاده کنیم؟**
    -   **کنترل توسط کاربر:** کاربر می‌تواند موقعیت ویجت شما را از طریق `config.js` تغییر دهد.
    -   **جلوگیری از همپوشانی:** سیستم به صورت خودکار از تداخل ویجت شما با سایر عناصر مانند نوار ناوبری جلوگیری می‌کند.
    -   **مدیریت واکنش‌گرایی:** رفتار ویجت شما در موبایل به راحتی قابل کنترل است.

-   **نحوه استفاده در `onInit`:**
    ```javascript
    onInit(app) {
      this.app = app;
      
      // 1. عنصر DOM خود را بسازید
      const myButton = document.createElement('button');
      myButton.textContent = 'Click Me';
      myButton.className = 'my-custom-button';

      // 2. اگر سیستم ویجت فعال است، آن را ثبت کنید
      if (this.app.config.widgets && this.app.config.widgets.enabled) {
        this.app.widgetManager.register({
          id: 'my-custom-button', // یک شناسه منحصر به فرد
          element: myButton,
        });
      } else {
        // 3. در غیر این صورت، از روش قدیمی برای موقعیت‌یابی استفاده کنید
    ...
    ```
> برای اطلاعات کامل در مورد این سیستم، به **[مستندات سیستم مدیریت ویجت‌ها](/docs/widgetSystem)** مراجعه کنید.
